...
from SinGAN.inpainting_functions import feed_net, mask
...
def train_single_scale(netD,netG,reals,Gs,Zs,in_s,NoiseAmp,opt,centers=None):
    ...
    for epoch in range(opt.niter):
        ...
        for j in range(opt.Dsteps):
            ...
            output = feed_net(netD, real, len(Gs), opt)
            ...
            output = feed_net(netD, fake, len(Gs), opt, detach = True) #maybe len(Gs) problem ?

            errD_fake = output.mean()
            errD_fake.backward(retain_graph=True)
            D_G_z = output.mean().item()

            gradient_penalty = functions.calc_gradient_penalty(netD, mask(real, len(Gs), opt), mask(fake, len(Gs), opt), opt.lambda_grad, opt.device)
        ...
        for j in range(opt.Gsteps):
            ...
            output = feed_net(netD, fake, len(Gs), opt)
            ...
            if alpha!=0:
                ...
                rec_loss = alpha*loss(mask(netG(Z_opt.detach(),z_prev), len(Gs), opt),mask(real, len(Gs), opt))
