...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        ...
        dir2save = '%s/Inpainting/%s/%s_out_%s' % (opt.out, opt.input_name[:-4],opt.mask_name[:-4], opt.patch_fill)
    return dir2save
...
def dilate_mask(mask,opt):
    if opt.mode == "harmonization":
        ...
        raise RuntimeError('Unkown mode in functions.dilate_mask.')
    ...
    if opt.mode == 'inpainting':
        plt.imsave('%s/%s_mask_dilated.png' % (opt.mask_dir, opt.mask_name[:-4]), convert_image_np(mask), vmin=0,vmax=1)
    else:
