# Inpainting using SinGAN

During this project I extend the application of SinGAN (https://arxiv.org/pdf/1905.01164.pdf) to inpainting. The SinGAN model code was taken from the official SinGAN github repo (https://tamarott.github.io/SinGAN.htm). In order to implement inpainting I had to create some new helper functions which are in the 'inpainting_functions.py' file. Otherwise I directly modified the code of the SinGAN model, mainly the 'train.py' and 'modelling.py'. I also created a new script 'inpainting.py' which allows the user to inpaint an image of their choice once the model has trained. The syntax to do so is as follows

## Inpainting use case

### Model Training

To train SinGAN model for inpainting on your own image, put the desired training image under Input/Images and the corresponding mask in Input/Masks, and run

```
python main_train.py --input_name <input_file_name> --inpainting
```

Failing to specify the option --inpainting will make you unable to run the inpainting script later on. To run this code on a cpu machine, specify `--not_cuda` when calling `main_train.py`

### Inpainting

To inpaint an image (see the example in the FinalReport.pdf) please first train the SinGAN model on the desired corrupted image and corruption mask, specifying the '--inpainting' flag. If one fails to specify this flag then the trained model will not be able to inpaint due to dimension issues. Now run the command

```
python inpainting.py --input_name <training_image_file_name> --mask_name <mask_image_file_name> --inpainting_start_scale <scale to inject> --patch_fill <fill type should be one of ['inpaint', 'average', 'neigh_average', 'none']>
```
...
## For the normal SinGAN function see the official README below
